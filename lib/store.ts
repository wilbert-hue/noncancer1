...
export const useDashboardStore = create<DashboardStore>((set, get) => ({
  ...
  updateFilters: (newFilters) => {
    ...
    set((state) => {
      // Volume mode removed from UI — always use value metrics
      ...
      if (newFilters.segmentType !== undefined && newFilters.segmentType !== state.filters.segmentType) {
        ...
        // Don't clear geographies when switching segment types - let user control geography selection
        const shouldClearGeographies = false
    ...
    set((state) => {
      const patch =
        newFilters.dataType === 'volume'
          ? { ...newFilters, dataType: 'value' as const }
          : newFilters
      const updatedFilters: FilterState = {
        ...state.opportunityFilters,
        ...(patch.geographies !== undefined && { geographies: patch.geographies || [] }),
        ...(patch.segments !== undefined && { segments: patch.segments || [] }),
        ...(patch.segmentType !== undefined && { segmentType: patch.segmentType || '' }),
        ...(patch.yearRange !== undefined && { yearRange: patch.yearRange || [2025, 2033] }),
        ...(patch.dataType !== undefined && { dataType: patch.dataType || 'value' }),
        ...(patch.viewMode !== undefined && { viewMode: patch.viewMode || 'segment-mode' }),
        ...(patch.businessType !== undefined && { businessType: patch.businessType }),
        ...(patch.aggregationLevel !== undefined && { aggregationLevel: patch.aggregationLevel }),
      }
